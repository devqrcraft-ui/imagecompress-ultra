...
for p in portals:
    ...
    os.makedirs(f"src/app/compress-for-{slug.replace(' ','-')}", exist_ok=True)
    with open(f"src/app/compress-for-{slug.replace(' ','-')}/page.tsx", "w", encoding="utf-8") as f:
        f.write(template.replace("[P]", p.replace("-"," ")).replace("[S]", slug.replace(" ","-")))
