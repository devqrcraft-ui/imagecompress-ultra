...
async function manualWorker() {
  const redis = new Redis({
    url: process.env.UPSTASH_REDIS_REST_URL,
    token: process.env.UPSTASH_REDIS_REST_TOKEN,
  });

  console.log("Manual Worker started...");

  const taskId = await redis.rpop("compression-queue");
  if (!taskId) { console.log("Queue is empty"); return; }
  console.log("Got task:", taskId);

  const taskData = await redis.get("task:" + taskId);
  if (!taskData) { console.log("Task not found"); return; }
  ...
  console.log("Done! Check: http://localhost:3000/api/task/" + taskId);
}

manualWorker().catch(console.error);
