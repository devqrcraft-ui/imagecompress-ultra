#!/bin/bash
echo "Creating 10 new landing pages..."

PAGES=(
  "compress-for-canva"
  "compress-for-google-classroom"
  "compress-for-discord"
  "compress-for-fiverr"
  "compress-for-upwork"
  "compress-for-twitter"
  "compress-for-pinterest"
...
for PAGE in "${PAGES[@]}"; do
  ...
  TITLE=$(echo $PAGE | sed 's/compress-for-//' | sed 's/-/ /g' | awk '{for(i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2); print}')
  PLATFORM=$(echo $PAGE | sed 's/compress-for-//')
  
cat > "src/app/$PAGE/page.tsx" << PAGEEOF
import { Metadata } from 'next';
import Link from 'next/link';

export const metadata: Metadata = {
  title: 'Compress Image for ${TITLE} — Free, No Upload | CompressTo20KB',
  description: 'Compress images for ${TITLE} free in your browser. No upload, no signup. Resize to exact KB for ${TITLE} requirements instantly.',
  keywords: ['compress image for ${PLATFORM}', '${PLATFORM} image size', '${PLATFORM} photo requirements', 'compress for ${PLATFORM} free'],
  alternates: { canonical: 'https://compressto20kb.com/${PAGE}' },
  openGraph: {
    title: 'Compress Image for ${TITLE} — Free & Private',
    description: 'Browser-based compression for ${TITLE}. Files never leave your device.',
    url: 'https://compressto20kb.com/${PAGE}',
  },
};

export default function CompressFor${TITLE//' '/''}Page() {
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)', color: 'white', fontFamily: 'system-ui,sans-serif' }}>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '48px 16px 24px' }}>
        <div style={{ fontSize: '13px', opacity: 0.5, marginBottom: '12px' }}><Link href="/blog" style={{ color: 'inherit', textDecoration: 'none' }}>Blog</Link> → Compress for ${TITLE}</div>
        <div style={{ display: 'inline-block', background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: '10px', padding: '6px 14px', fontSize: '12px', marginBottom: '16px' }}>🆓 ${TITLE} · Free Tool</div>
        <h1 style={{ fontSize: 'clamp(24px,4vw,40px)', fontWeight: 900, margin: '0 0 16px', lineHeight: 1.2 }}>Compress Images for ${TITLE}<br/><span style={{ color: '#818cf8' }}>Free, Private & Instant</span></h1>
        <p style={{ fontSize: '16px', opacity: 0.65, marginBottom: '32px', lineHeight: 1.7 }}>Need to compress images for ${TITLE}? Our free browser-based tool compresses JPEG, PNG, WebP instantly — no upload required, no signup needed. Your files never leave your device.</p>
        ...
        <div style={{ background: 'rgba(99,102,241,0.15)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: '16px', padding: '24px', marginBottom: '32px', textAlign: 'center' }}>
          <p style={{ fontSize: '18px', fontWeight: 700, marginBottom: '12px' }}>Ready to compress for ${TITLE}?</p>
        ...
        <h2 style={{ fontSize: '20px', fontWeight: 800, marginBottom: '16px' }}>${TITLE} Image Requirements</h2>
        ...
        <h2 style={{ fontSize: '20px', fontWeight: 800, marginBottom: '16px' }}>How to Compress Images for ${TITLE} (3 Steps)</h2>
        <div style={{ display: 'grid', gap: '12px', marginBottom: '32px' }}>
          {[
            { n: '1', t: 'Upload Your Image', d: 'Drag & drop or click to select. Supports JPEG, PNG, WebP, HEIC up to 50MB.' },
            { n: '2', t: 'Set Target Size', d: 'Type your target KB — or use the ${TITLE} preset for instant optimization.' },
            { n: '3', t: 'Download Compressed File', d: 'Click download. Your optimized image is ready for ${TITLE} in seconds.' },
          ].map(s => (
        ...
        <h2 style={{ fontSize: '20px', fontWeight: 800, marginBottom: '16px' }}>Why Use CompressTo20KB for ${TITLE}?</h2>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '24px' }}>
          <h2 style={{ fontSize: '18px', fontWeight: 800, marginBottom: '16px' }}>FAQ — Compress Images for ${TITLE}</h2>
          {[
            { q: 'Is this tool really free for ${TITLE}?', a: 'Yes, 100% free. No signup, no subscription, no hidden fees.' },
            ...
            { q: 'How small can I compress images for ${TITLE}?', a: 'You can compress to any target size — from 5KB to 5MB. Just type your desired file size.' },
          ].map((f, i) => (
        ...
        <div style={{ textAlign: 'center', marginTop: '40px', padding: '32px', background: 'rgba(99,102,241,0.1)', borderRadius: '16px', border: '1px solid rgba(99,102,241,0.2)' }}>
          <h2 style={{ fontSize: '22px', fontWeight: 800, marginBottom: '8px' }}>Start Compressing for ${TITLE} Now</h2>
