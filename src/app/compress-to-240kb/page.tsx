...
export const metadata: Metadata = {
  title: 'Compress Image to 240KB — Free Online Tool',
  description: 'Compress any image to exactly 240KB online. Perfect for DS-160 US Visa form. Free, no upload, 100% private. Works instantly in your browser.',
  alternates: { canonical: '/compress-to-240kb' },
  keywords: 'compress image to 240KB, reduce photo to 240KB, compress to 240KB, image compressor 240KB',
};
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)', color: 'white', fontFamily: 'system-ui,sans-serif' }}>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        '@type': 'WebApplication',
        name: 'Compress Image to 240KB',
        url: 'https://compressto20kb.com/compress-to-240kb',
        description: 'Free tool to compress images to exactly 240KB.',
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '48px 16px 24px' }}>
        <h1 style={{ fontSize: 'clamp(26px,4vw,42px)', fontWeight: 900, lineHeight: 1.2, marginBottom: '12px' }}>
          📦 Compress Image to <span style={{ color: '#818cf8' }}>240KB</span>
        </h1>
        <p style={{ fontSize: '15px', lineHeight: 1.7, opacity: 0.7, marginBottom: '32px' }}>
          Instantly compress any photo or image to exactly 240KB. Perfect for DS-160 US Visa form. No upload required — all processing happens in your browser.
        </p>
        <div style={{ background: 'rgba(129,140,248,0.12)', border: '1px solid rgba(129,140,248,0.35)', borderRadius: '14px', padding: '24px', marginBottom: '40px', textAlign: 'center' }}>
          <div style={{ fontSize: '15px', fontWeight: 700, marginBottom: '8px' }}>🎯 Compress to exactly 240KB</div>
          ...
          <a href={"/?mode=exactkb&target=240"} style={{ display: 'inline-block', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: 'white', padding: '12px 28px', borderRadius: '100px', fontWeight: 800, fontSize: '14px', textDecoration: 'none' }}>
            ⚡ Open Compressor — Target 240KB
          </a>
        </div>
        <h2 style={{ fontSize: '22px', fontWeight: 700, marginBottom: '16px' }}>How to compress image to 240KB</h2>
        {[
          ...
          { step: '4', title: 'Download compressed file', desc: 'Your image is compressed to under 240KB. Download instantly.' },
        ].map(({ step, title, desc }) => (
