      ...
      <div style={{display:'flex',gap:'8px',marginBottom:'16px',flexWrap:'wrap'}}>
        {FMTS.map(([f,label,sub])=>(
          <button key={f} onClick={()=>setFormat(f)} style={{padding:'8px 14px',borderRadius:'8px',border:`1px solid ${format===f?'#6366f1':'rgba(255,255,255,0.1)'}`,background:format===f?'rgba(99,102,241,0.25)':'rgba(255,255,255,0.04)',color:'white',cursor:'pointer',fontSize:'12px'}}>
            <div style={{fontWeight:700}}>{label}</div><div style={{opacity:0.55,fontSize:'10px'}}>{sub}</div>
          </button>
        ))}
      </div>
      {mode==='quality'?(
        <div style={{marginBottom:'16px'}}>
          ...
          <input type="range" min={10} max={100} value={quality} onChange={e=>setQuality(+e.target.value)} style={{width:'100%',accentColor:'#6366f1'}}/>
        </div>
      ):(
        <div style={{marginBottom:'16px'}}>
          ...
          <span style={{fontSize:'12px',opacity:0.5,marginLeft:'8px'}}>KB</span>
        </div>
      )}
      ...
      {images.length>0&&(
        <div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}>
            <span style={{fontSize:'13px',opacity:0.6}}>{images.length} image{images.length>1?'s':''}</span>
          ...
          {images.map(img=>(
            <div key={img.id} style={{display:'flex',alignItems:'center',gap:'10px',padding:'10px 12px',background:'rgba(255,255,255,0.04)',borderRadius:'10px',marginBottom:'6px',border:'1px solid rgba(255,255,255,0.07)'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:'11px',opacity:0.5,marginTop:'2px'}}>{fmtSize(img.origSize)}{img.compSize&&<span style={{color:'#34d399'}}> → {fmtSize(img.compSize)} <span style={{fontWeight:700}}>(-{img.saved}%)</span></span>}</div>
              </div>
              <div style={{flexShrink:0}}>
                {img.status==='pending'&&<span style={{opacity:0.35,fontSize:'13px'}}>⏸️ Ready</span>}
